...
# disable environment proxies to hit healthcheck directly
...
class APITestCase(unittest.TestCase):
    ...
    #     data = r.json()
    #     self.assertIn('apiKey', data)
    #     self.__class__.generated_api_key = data['apiKey']

    # def test_05_admin_update_subscription(self):
    ...
    # def test_06_create_launch_token_unauthorized(self):
    #     url = urljoin(BASE_URL, "/api/create-launch-token")
    #     r = session.get(url)
    #     self.assertEqual(r.status_code, 401)

    # def test_07_create_launch_token(self):
    #     url_up = urljoin(BASE_URL, "/api/update-subscription-status")
    #     session.post(url_up, headers={'X-Admin-Secret': ADMIN_SECRET},
    #                  json={'userIdentifier': TEST_USER_IDENTIFIER, 'subscriptionStatus': 'active'})
